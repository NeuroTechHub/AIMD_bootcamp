...
MODULE_CONTENT = {
    ...
    "M3-neuromod-and-stim.html": dict(
        eyebrow="NTH / M3 &middot; Interactive companion",
        h1='Neuromodulation <span style="color:var(--muted-2);font-weight:500">&amp; stimulation</span>',
        lead="antonio",
        status="release",
        colab="https://colab.research.google.com/github/NeuroTechHub/AIMD_bootcamp/blob/main/modules/M3-neuromod-and-stim/neuromod-and-stim.ipynb",
        lede=("How a cortical prosthesis turns numbers into neuronal firing. "
              ...
              "one real-time stimulator with safety chips and a live charge-rate readout &mdash; let you build intuition "
        ...
        sections=[
            ...
            ("04", "neurolight",  "Run the stimulator",
             ...
             "All four can be read at a glance: <em>where</em> firing is happening (array), <em>when</em> it happens (carousel), <em>how safe</em> the parameters are (current limit + Shannon-k), and <em>how fast</em> charge is being delivered (peak current per channel + instant charge-rate trace)."),
        ...
        footer=("NTH bootcamp &middot; M3 &middot; "
                ...
                "&middot; safety helpers implemented in <code>~400 lines of vanilla JS</code> below; "
                'for the Python version see the '
                '<a href="https://colab.research.google.com/github/NeuroTechHub/AIMD_bootcamp/blob/main/modules/M3-neuromod-and-stim/neuromod-and-stim.ipynb" target="_blank" rel="noopener">companion notebook</a>.'),
    ),
...
(function(){
  ...
  surpriseBtn.addEventListener('click', () => {
    // refuse if the lifecycle isn't at idle (connect must be available)
    const fd0 = document.getElementById('flow-demo');
    const connBtn0 = fd0 && fd0.querySelector('#fd-btn-connect');
    if(connBtn0 && connBtn0.disabled){
...
<div class='flow-demo conductor' id='flow-demo'>
  ...
  <div class='cond-header'>
    <div class='cond-pills' role='group' aria-label='Lifecycle steps'>
      <button class='cond-pill armed' data-step='0' id='fd-btn-connect'>
        <span class='n'>1</span><span class='lbl'>Connect</span>
      </button>
      <span class='cond-arrow'>&rsaquo;</span>
      <button class='cond-pill' data-step='1' id='fd-btn-configure' disabled>
        <span class='n'>2</span><span class='lbl'>Configure</span>
      </button>
      <span class='cond-arrow'>&rsaquo;</span>
    ...
    <div class='cond-cell cond-safety'>
      ...
      <div class='cond-safety-grid' id='fd-safety'>
        ...
        <div class='fd-metric live'>
          <div class='fd-metric-h'>instant charge rate</div>
          <div class='fd-metric-v' id='fd-mx-P'>0.00 &micro;C/s</div>
          <div class='fd-metric-sub'>now &middot; sum of channels</div>
        </div>
      </div>
      <div class='fd-safety-warn' id='fd-safety-warn'></div>
    </div>

    <!-- BR: instant charge-rate trace -->
    <div class='cond-cell cond-power'>
      <div class='cond-h'>
        <span>Instant charge-rate trace</span>
        <span class='cond-tag'>analytic, all channels</span>
        <span class='cond-sub' id='fd-power-now'>0.00 &micro;C/s</span>
      </div>
      <div class='cond-power-body'>
        <svg id='fd-power-svg' viewBox='0 0 1000 180' aria-label='Instant charge-rate live chart' preserveAspectRatio='none'></svg>
      </div>
    </div>
  ...
  <details class='cond-log-wrap'>
    ...
    <div class='fd-log' id='fd-log'>
      <div class='fd-log-row muted'><span class='ts'>--:--.---</span><span class='msg'>stimulator idle &middot; press Connect to begin</span></div>
    </div>
  </details>
...
(function(){
  ...
  const LIFECYCLE = ['connect', 'configure', 'stim', 'disconnect'];
  ...
  function setButtons(active){
    const order = ['conn','cfg','stim','disc'];
    order.forEach(name => {
      const b = btn[name];
    ...
    if(phase === 0){
      btn.cfg.disabled = false; btn.cfg.classList.add('armed');
    } else if(phase === 1){
  ...
  function updateState(){
    ...
    setButtons();
    if(phase < 0){
      summaryEl.innerHTML = 'press connect to begin';
    } else if(phase === 0){
      summaryEl.innerHTML = 'connected &middot; build a channel list above and configure';
    } else if(phase === 1){
      summaryEl.innerHTML = channels.length + ' channels armed &middot; press stim to deliver';
  ...
  // SVG handles for the instant-rate trace + summary readout.
  ...
  // diagnostic that matters is the rate of charge delivery: peak current per
  // channel, plus an instantaneous &micro;C/s trace that rises during a train
  // and drops to zero in the inter-train gaps. nPulsesFired() and
  // cumulativeChargeUc() are kept for completeness (used by other diagnostics
  // and the self-check math), even though the live chart now shows the rate.
  ...
  // historical buffer for the instant-rate trace
  const traceBuf = []; // [{t, q}]   q in uC/s (instantaneous charge rate)
  const TRACE_HISTORY_MS = 12000;
  // Fixed y-scale for the rate chart. Set once at stim start to the peak
  // steady-state rate (sum of chargePerPhase * freq across channels) so the
  // bars during a train read full-height without the axis rescaling on every
  // train gap.
  let chartYMaxUc = 0.01;
  // Channel is firing pulses at time t iff we're inside one of its trains.
  function channelInTrain(ch, t){
    if(runStartMs == null || t < runStartMs || t > runEndMs) return false;
    const p = ch.params;
    const trainMs = trainDurationMs(p);
    const trainPeriodMs = effectiveTrainPeriodMs();
    const n = +reps.value;
    const elapsed = t - runStartMs;
    const trainIdx = Math.floor(elapsed / trainPeriodMs);
    if(trainIdx >= n) return false;
    const localT = elapsed - trainIdx * trainPeriodMs;
    return localT >= 0 && localT <= trainMs;
  }
  function instantChargeRateUcPerSec(t){
    if(channels.length === 0 || runStartMs == null) return 0;
    if(t < runStartMs || t > runEndMs) return 0;
    let rate = 0;
    for(const ch of channels){
      if(channelInTrain(ch, t)){
        rate += chargePerPhaseUc(ch.params) * ch.params.fr;
      }
    }
    return rate;
  }
  function peakInstantRateUcPerSec(){
    let m = 0;
    for(const ch of channels){
      m += chargePerPhaseUc(ch.params) * ch.params.fr;
    }
    return m;
  }
  function niceCeil(x){
  ...
  function renderChargeChart(nowMs){
    ...
    out += '<text x="'+(padL-8)+'" y="'+(padT+10)+'" text-anchor="end" font-family="JetBrains Mono, monospace" font-size="10" fill="#6c6c66">'+ fmtFix(yMax,2) +' &#xB5;C/s</text>';
    out += '<text x="'+(padL-8)+'" y="'+(padT+plotH-2)+'" text-anchor="end" font-family="JetBrains Mono, monospace" font-size="10" fill="#6c6c66">0</text>';

    // instant-rate area: filled polygon down to baseline so train gaps read clearly.
    if(traceBuf.length){
      ...
      for(let i = 0; i < traceBuf.length; i++){
        ...
        if(s.t > tEnd)   break;
        const x = xs(s.t).toFixed(1);
        if(firstX === null) firstX = x;
        lastX = x;
        pts += x + ',' + ys(s.q).toFixed(1) + ' ';
      }
      if(pts){
        const filled = firstX + ',' + yBase + ' ' + pts + lastX + ',' + yBase;
        out += '<polygon points="'+filled+'" fill="rgba(216,111,145,0.18)" stroke="none"/>';
        out += '<polyline points="'+pts+'" fill="none" stroke="#d86f91" stroke-width="1.8"/>';
      }
    }
    ...
    out += '<text x="'+(padL+plotW/2)+'" y="'+(H-4)+'" text-anchor="middle" font-family="JetBrains Mono, monospace" font-size="10" fill="#6c6c66">'+
            (windowMs/1000).toFixed(2) + ' s window &middot; instantaneous rate &middot; bars rise during trains, fall in the gaps</text>';

    powerSvg.innerHTML = out;
  }

  function renderInstantCharge(nowMs){
    const rateNow = instantChargeRateUcPerSec(nowMs);
    traceBuf.push({t: nowMs, q: rateNow});
    const cutoff = nowMs - TRACE_HISTORY_MS;
    while(traceBuf.length && traceBuf[0].t < cutoff) traceBuf.shift();
    mx.P.innerHTML = fmtFix(rateNow, 2) + ' &micro;C/s';
    powerNowEl.innerHTML = fmtFix(rateNow, 2) + ' &micro;C/s';
  ...
  btn.conn.addEventListener('click', () => {
    if(phase >= 0) return;
    phase = 0;
    logLine('connect &middot; stimulator online &middot; impedance check ok', 'info');
    updateState();
  });
  ...
  btn.cfg.addEventListener('click', () => {
    ...
    channels = chs;
    phase = 1;
    logLine('configure &middot; ' + chs.length + ' channel' + (chs.length>1?'s':'') + ' armed', 'info');
  ...
  function startStimRun(){
    ...
    // lock the rate-chart y-scale to the projected peak rate (sum of
    // chargePerPhase * freq across channels) so the bars during a train
    // read full-height regardless of duty cycle.
    traceBuf.length = 0;
    chartYMaxUc = niceCeil(Math.max(1e-3, peakInstantRateUcPerSec()));
...
<aside class="callout"><strong>Safety, in passing.</strong>
We compute <code>k = log<sub>10</sub>(Q) + log<sub>10</sub>(Qd)</code> per channel &mdash; charge per
phase <code>Q</code> in &micro;C, charge density <code>Qd</code> in &micro;C/cm&sup2;. Above
<code>k &approx; 1.85</code> Shannon (1992) flagged a damage-risk corner. Treat it as a
<strong>caution flag, not a pass/fail gate</strong>.
<details style="margin-top:10px">
  <summary style="cursor:pointer;color:var(--ink);font-weight:500">Why <code>k &le; 1.85</code> is only a flag</summary>
  <p style="margin:8px 0 0">Shannon's 1992 fit comes from <em>large surface electrodes</em> in chronic
  animal work &mdash; areas orders of magnitude bigger than the 2000&nbsp;&micro;m&sup2; intracortical
  site simulated here. Penetrating microelectrodes routinely operate at charge densities well above
  the line, and modern reviews (Cogan&nbsp;2008; Cogan, Ludwig, Welle&nbsp;&amp; Takmakov&nbsp;2016)
  treat tissue-damage thresholds as electrode-, waveform-, and duty-cycle-dependent rather than a
  single universal cutoff. See the references at the foot of the page.</p>
</details></aside>
"""
...
<ul>
  ...
  13(2):021001. <a href="https://doi.org/10.1088/1741-2560/13/2/021001" target="_blank" rel="noopener">doi:10.1088/1741-2560/13/2/021001</a>
  &mdash; why microelectrode safety limits are electrode-, waveform-, and duty-cycle-dependent
  rather than a single universal cutoff.</li>
...
<h3>Further reading &mdash; vision-restoration field</h3>
<ul>
  <li><span class="rk">overview</span> <a href="../docs/vision-restoration-field.md" target="_blank" rel="noopener">docs/vision-restoration-field.md</a> &mdash; programs by tissue (retina &middot; LGN &middot; V1), with links to each company / consortium. The configure-array workflow you just used mirrors the V1 cortical-implant cohorts most directly.</li>
  <li><span class="rk">paper</span> Fern&aacute;ndez, Alfaro, Soto-S&aacute;nchez, <em>et al.</em> (2021), <em>Visual percepts evoked with an intracortical 96-channel microelectrode array inserted in human occipital cortex</em>, J. Clinical Investigation 131(23):e151331. <a href="https://doi.org/10.1172/JCI151331" target="_blank" rel="noopener">doi:10.1172/JCI151331</a> &mdash; the human Utah-array cohort whose pulse-parameter ranges this module's defaults sit inside.</li>
  <li><span class="rk">paper</span> Granley &amp; Beyeler (2021), <em>A computational model of phosphene appearance for epiretinal prostheses</em>, IEEE EMBC. <a href="https://doi.org/10.1109/EMBC46164.2021.9629663" target="_blank" rel="noopener">doi:10.1109/EMBC46164.2021.9629663</a> &mdash; how pulse-amplitude / width / frequency map to perceived size and brightness; the parameter space M4 picks up from here.</li>
  <li><span class="rk">program</span> <a href="https://cortivis.umh.es/" target="_blank" rel="noopener">CORTIVIS (Fern&aacute;ndez / UMH Elche)</a> &mdash; active first-in-human cortical-implant trial; the configure &rarr; connect &rarr; stim lifecycle of this module is modelled on the bedside protocol.</li>
</ul>
"""
...
""")
    ...
    _status = spec.get('status', 'stub')
    status_html = (
        f"<span><strong>Status:</strong> {_status}</span>" if _status != "release" else ""
    )
    colab_url = spec.get('colab')
    colab_html = (
        f'<span><strong>Notebook:</strong> <a href="{colab_url}" target="_blank" rel="noopener">Colab</a></span>'
        if colab_url else ""
    )
    meta = (
