...
**Round 4 — CSS extraction** (per user OK):

- Extract shared rules to `modules/_shared.css` linked from every HTML.
- Caveat: `bootcamp-plan.html` and `modules/M3-neuromod-and-stim.html` are generated by `build/_build_bootcamp.py`. Either also update the builder, or pin those two to inline CSS for now and document the exception in the builder header.
