...
The HTML modules are browser-only. The optional notebooks need Python packages:

```bash
python -m pip install -r requirements-notebooks.txt
```

Each notebook still includes a Colab-friendly `%pip install` cell, but
`requirements-notebooks.txt` is the shared local dependency list and should be
updated first when notebook dependencies change.

Run the lightweight repo check before handing off:

```bash
python build/validate_notebooks.py
```

Run the heavy solution execution pass when the ML dependencies and downloads are
available. This executes the core solution notebooks (M1, M3, M4, M5):

```bash
python build/validate_notebooks.py --execute-solutions
```

Add `--execute-all-solutions` when you also want to run the heavier M2
DeepGaze notebook.
